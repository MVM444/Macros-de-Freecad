...
def test_demo_two_storey_mode_reuses_native_level_and_existing_services():
    text = (ROOT / "commands" / "cmd_demo_building.py").read_text(encoding="utf-8")
    assert '"two_storey"' in text
    assert "build_two_storey_demo_spec" in text
    assert "def _materialize_two_storey(spec):" in text
    assert "ensure_bim_structure(" in text
    assert 'level_name=upper_info["level_name"]' in text
    assert 'create_site=False' in text
    assert "create_walls_from_centerline_sketches" in text
    assert "create_openings_from_centerlines" in text
    assert "create_bim_spaces" in text
    assert "create_modular_ceilings" in text
...
def test_two_storey_v080_uses_absolute_z_distinct_levels_and_canonical_native_stair():
    text = (ROOT / "commands" / "cmd_demo_building.py").read_text(encoding="utf-8")
    assert 'DEMO_COMMAND_VERSION = "0.9.3"' in text
    assert "create_level_if_label_missing=self.create_level_if_label_missing" in text
    assert 'ceiling_namespace="Nivel00"' in text
    assert 'ceiling_namespace="Nivel01"' in text
    assert 'FA_Ceilings_%s' in text
    assert 'Spreadsheet_CielosSuspendidos_%s' in text
    assert "Jerarquia BIM verificada | Levels nativos=2" in text
    assert "def _confirm_long_process_notice(mode=\"fixed\"):" in text
    assert 'i18n.bi("Generar", "Generate")' in text
    assert 'i18n.bi("Cancelar", "Cancel")' in text
    assert "return box.clickedButton() is generate_button" in text
    assert "if not _confirm_long_process_notice" in text
    assert "def _elevate_completed_storey" not in text
    assert "geometry_z_offset_mm=upper_z" in text
    assert '"floor_top_z_mm": float(self.spec["floor"]["top_z_mm"]) + self.geometry_z_offset_mm' in text
    assert "z_mm=self.geometry_z_offset_mm" in text
    assert "_validate_storey_wall_z(upper, upper_z)" in text
    assert text.index("_validate_storey_wall_z(upper, upper_z)") < text.index('for number in range(5, 14):')
    assert 'openTransaction("FA Demo 2 pisos - Nivel 01 estructura")' in text
    assert 'openTransaction("FA Demo 2 pisos - Nivel 01 aberturas y techo")' in text
    assert "plan_angled_stair" in text
    assert "build_stair_context" in text
    assert "create_native_stair" in text
    assert "plan_stair_clearance" in text
    assert "create_clearance_previews" in text
    assert "create_native_slab_opening" in text
    assert "create_stair_opening_liner" in text
    assert "Tapichel buque escalera" in text
    assert "clearance_geometry_revision" in text
    assert "opening_shape" in text
    assert "skipped_edge_count" in text
    assert "StairOpeningLiner" in text
    liner_call = text.split("liner_result = create_stair_opening_liner(", 1)[1].split('stair_result["opening_liner"]', 1)[0]
    assert 'plane_id="lower_ceiling"' in liner_call
    assert "level=ground.level" in liner_call
    assert "ceiling_plane_id=" not in liner_call
    assert "target_container=" not in liner_call
    assert "mark_clearance_plans_applied" in text
    assert "exclusion_zones_world_mm" in text
    assert "native_subtraction_applied" in text
    assert "generator_exclusion_applied" in text
    assert "def _materialize_demo_stair(" in text
    assert "Part.makePolygon(points)" in text
    assert "Part.makeBox" not in text.split("def _materialize_demo_stair(", 1)[1].split("def _materialize_two_storey", 1)[0]
    assert 'railings_mode=str(stair_spec.get("railings_mode", "native"))' in text
    assert 'prevent_duplicate=True' in text
    assert '"StairStatus", "Demo", "Estado de escalera nativa", "created_native"' in text
    assert "StairSlabOpeningPreview" in text
    assert "StairCeilingExclusionPreview" in text
    assert '"StairClearanceStatus", "Demo", "Estado de holgura", "applied"' not in text or "clearance_ok" in text


def test_demo_v081_generates_full_document_diagnostic_and_reuses_result_dialog():
    text = (ROOT / "commands" / "cmd_demo_building.py").read_text(encoding="utf-8")
    assert 'DEMO_COMMAND_VERSION = "0.9.3"' in text
    assert "from .cmd_model_diagnostic import generate_report, show_report_dialog" in text
    assert "def _run_demo_diagnostic(" in text
    assert "selection=[]" in text
    assert 'objective="Validacion automatica de FA Demo edificio"' in text
    assert "copy_prompt=False" in text
    assert "show_report_dialog(" in text
    assert '_run_demo_diagnostic(\n                result["document"]' in text
    assert "self._diagnostic_generated = False" in text
    assert "number == guided_total_steps() and not self._diagnostic_generated" in text
