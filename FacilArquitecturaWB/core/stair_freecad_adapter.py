...
GENERATED_BY = "FA_StairBetweenSlabs_v0.6.5"
...
def _make_base_objects(doc, plan, geometry_mode="part_feature", local_frame_source=None):
    """Create the three native Arch-Stairs bases.

    ``geometry_mode='draft_line_wire'`` is an opt-in path used by the minimal
    stair demo.  It keeps the actual drawing primitives as Draft Line/Wire and
    stores their geometry in the local frame of ``local_frame_source``.  The
    historical Part::Feature/world-coordinate behaviour remains the default for
    the interactive stair tool and the two-storey house demo.
    """
    geom = plan["geometry"]
    mode = str(geometry_mode or "part_feature").strip().lower()

    if mode != "draft_line_wire":
        ...
        frame = _global_placement(local_frame_source) if local_frame_source is not None else App.Placement()
        inv = frame.inverse()

        def local(values):
            return inv.multVec(_vec(values))

        make_line = getattr(Draft, "make_line", None) or getattr(Draft, "makeLine", None)
        make_wire = getattr(Draft, "make_wire", None) or getattr(Draft, "makeWire", None)
        if not callable(make_line) or not callable(make_wire):
            raise RuntimeError("Draft no expone make_line/make_wire para las bases de escalera.")

        f1 = make_line(local(geom["flight1_start_xyz"]), local(geom["flight1_end_xyz"]))
        f1.Label = "FA Escalera - Base tramo 1"
        landing = make_wire([local(point) for point in geom["landing_xyz"]], closed=False, face=False)
        landing.Label = "FA Escalera - Base descanso"
        f2 = make_line(local(geom["flight2_start_xyz"]), local(geom["flight2_end_xyz"]))
        f2.Label = "FA Escalera - Base tramo 2"

    for obj in (f1, landing, f2):
        set_prop(obj, "App::PropertyString", "FA_GeneratedBy", "FacilArquitectura", "Generador", GENERATED_BY)
        set_prop(obj, "App::PropertyString", "FA_BaseGeometryMode", "FacilArquitectura", "Tipo de geometria base", mode)
...
def _tag_master(master, context, plan, railings_mode="native", context_links_mode="links"):
    lower = context["lower"]
    upper = context["upper"]
    source = context["source"]
    lower_level = context.get("lower_level")
    upper_level = context.get("upper_level")
    mode = str(context_links_mode or "links").strip().lower()
    ...
    if mode == "names":
        # Minimal-demo master-placement mode: a cutter driven by master.Placement
        # would close a dependency cycle if master also PropertyLinks the Levels/
        # slabs that contain that cutter. Preserve semantic identity by stable Names
        # while keeping the production/default link contract unchanged.
        props.extend((
            ("App::PropertyString", "FA_LowerSlabName", str(getattr(lower, "Name", "") or "")),
            ("App::PropertyString", "FA_UpperSlabName", str(getattr(upper, "Name", "") or "")),
            ("App::PropertyString", "FA_LowerLevelName", str(getattr(lower_level, "Name", "") or "")),
            ("App::PropertyString", "FA_UpperLevelName", str(getattr(upper_level, "Name", "") or "")),
        ))
    else:
        props.extend((
...
def _make_plan2d(doc, master, plan, local_frame_source=None):
    ...
    if local_frame_source is not None:
        inv = _global_placement(local_frame_source).inverse()
        world_z = float(z)

        def local_xy(point):
            vec = inv.multVec(App.Vector(float(point[0]), float(point[1]), world_z))
            return [float(vec.x), float(vec.y)]

        z = float(inv.multVec(App.Vector(float(p0[0]), float(p0[1]), world_z)).z)
        p0, pc, p2 = local_xy(p0), local_xy(pc), local_xy(p2)
        landing_start, landing_end = local_xy(landing_start), local_xy(landing_end)
    ...
    set_prop(obj, "App::PropertyString", "FA_GeneratedBy", "FacilArquitectura", "Generador", GENERATED_BY)
    if local_frame_source is not None:
        set_prop(obj, "App::PropertyLink", "FA_PlacementSource", "FacilArquitectura", "Fuente de Placement", local_frame_source)
        try:
            obj.setExpression("Placement", "%s.Placement" % local_frame_source.Name)
        except Exception as exc:
            raise RuntimeError("No se pudo enlazar el PLAN 2D al Placement local de la escalera: %s" % exc)
    try:
...
def create_native_slab_opening(
    ...
    metadata = {
        ...
        "z_min_mm": z_min,
        "z_max_mm": z_max,
        "placement_source": getattr(placement_source, "Name", "") if placement_source is not None else "",
    }
    if bool(dry_run):
        return metadata
    ...
    if placement_source is not None:
        # FreeCAD 1.1.3 does not guarantee that an Arch subtraction follows the
        # transform of another BIM object.  Build the cutter in the same local
        # frame as the stair source and drive its Placement from that source.
        inv = _global_placement(placement_source).inverse()
        local_zones = []
        for zone in zones:
            local_zone = dict(zone)
            polygon = []
            for x, y in list(zone.get("polygon_mm", []) or []):
                point = inv.multVec(App.Vector(float(x), float(y), z_min))
                polygon.append([float(point.x), float(point.y)])
            local_zone["polygon_mm"] = polygon
            local_zones.append(local_zone)
        probe_xy = list((zones[0].get("polygon_mm", []) or [[0.0, 0.0]])[0])
        local_min = inv.multVec(App.Vector(float(probe_xy[0]), float(probe_xy[1]), z_min))
        local_max = inv.multVec(App.Vector(float(probe_xy[0]), float(probe_xy[1]), z_max))
        shape_zones = local_zones
        shape_z_min = float(local_min.z)
        shape_z_max = float(local_max.z)

    cutter.Shape = _fused_clearance_solid(shape_zones, shape_z_min, shape_z_max)
    ...
    if placement_source is not None:
        ...
        set_prop(cutter, "App::PropertyString", "FA_LocalClearanceZonesJSON", "FacilArquitectura", "Zonas locales 2D JSON", json.dumps(shape_zones, sort_keys=True, separators=(",", ":")))
        try:
            cutter.setExpression("Placement", "%s.Placement" % placement_source.Name)
        except Exception as exc:
            raise RuntimeError("No se pudo enlazar el buque al Placement de la escalera: %s" % exc)
    ...
    if master is not None:
        if bool(link_master_opening):
            set_prop(master, "App::PropertyLink", "FA_SlabOpening", "FacilArquitectura", "Buque nativo en losa superior", cutter)
        else:
            # In master-placement mode the cutter depends on master.Placement. A
            # reverse master -> cutter PropertyLink would create a direct cycle.
            set_prop(master, "App::PropertyString", "FA_SlabOpeningName", "FacilArquitectura", "Nombre del buque nativo", str(getattr(cutter, "Name", "") or ""))
...
def build_ceiling_finish_exclusion_zones(
    ...
    for index, edge in enumerate(boundary_edges):
        if _edge_is_open_liner_end(edge, shape_open_ends, effective_width):
            continue
...
def create_stair_opening_liner(
    ...
    ``placement_source`` is opt-in and is used by Demo Escalera minima: the
    tapichel is built in the source local frame and its Placement follows that
    source.  ``link_master_liner=False`` avoids the reverse master -> liner link
    when the liner already depends on ``master.Placement``.  Defaults preserve
    the historical two-storey/demo and interactive behaviour.
    """
    ...
    shape_zones = zones
    shape_open_ends = open_ends
    shape_z_min = z_min
    shape_z_max = z_max
    if placement_source is not None:
        # Build the tapichel in the same local frame used by the movable stair.
        # This mirrors the slab-opening strategy: geometry is local, Placement is
        # the only moving authority.  Entry/exit descriptors must be transformed
        # too, otherwise the open ends would be detected at the original world XY.
        inv = _global_placement(placement_source).inverse()
        local_zones = []
        for zone in zones:
            local_zone = dict(zone)
        ...
        local_open_ends = []
        for item in open_ends:
            local_item = dict(item)
            point = list(item.get("point_mm", []) or [])
            if len(point) >= 2:
                transformed = inv.multVec(App.Vector(float(point[0]), float(point[1]), z_min))
                local_item["point_mm"] = [float(transformed.x), float(transformed.y)]
            direction = list(item.get("direction_xy", []) or [])
            if len(direction) >= 2:
                try:
                    transformed_dir = inv.Rotation.multVec(
                        App.Vector(float(direction[0]), float(direction[1]), 0.0)
                    )
                    local_item["direction_xy"] = [float(transformed_dir.x), float(transformed_dir.y)]
                except Exception:
                    pass
            local_open_ends.append(local_item)

        probe = list((zones[0].get("polygon_mm", []) or [[0.0, 0.0]])[0])
        local_min = inv.multVec(App.Vector(float(probe[0]), float(probe[1]), z_min))
        local_max = inv.multVec(App.Vector(float(probe[0]), float(probe[1]), z_max))
        shape_zones = local_zones
        shape_open_ends = local_open_ends
        shape_z_min = float(local_min.z)
        shape_z_max = float(local_max.z)

    opening_face = _fused_clearance_face(shape_zones, shape_z_min)
    if opening_face.isNull() or not list(getattr(opening_face, "Faces", []) or []):
    ...
    for edge in boundary_edges:
        if _edge_is_open_liner_end(edge, open_ends, effective_width):
            skipped_edges += 1
            continue
        solid = _outside_liner_segment(opening_face, edge, thickness, shape_z_min, shape_z_max)
        if solid is not None and not solid.isNull():
            solids.append(solid)
    if not solids:
    ...
    try:
        clear_void = opening_face.extrude(App.Vector(0.0, 0.0, shape_z_max - shape_z_min))
    ...
    liner = None
    if master is not None:
        if bool(link_master_liner):
            liner = getattr(master, "FA_OpeningLiner", None)
        else:
            liner_name = str(getattr(master, "FA_OpeningLinerName", "") or "")
            liner = doc.getObject(liner_name) if liner_name else None
    ...
    if placement_source is not None:
        set_prop(liner, "App::PropertyLink", "FA_PlacementSource", "FacilArquitectura", "Fuente de Placement", placement_source)
        set_prop(
            liner,
            "App::PropertyString",
            "FA_LocalClearanceZonesJSON",
            "FacilArquitectura",
            "Zonas locales 2D JSON",
            json.dumps(shape_zones, sort_keys=True, separators=(",", ":")),
        )
        set_prop(
            liner,
            "App::PropertyString",
            "FA_LocalOpenEndsJSON",
            "FacilArquitectura",
            "Extremos abiertos locales JSON",
            json.dumps(shape_open_ends, sort_keys=True, separators=(",", ":")),
        )
        try:
            liner.setExpression("Placement", "%s.Placement" % placement_source.Name)
        except Exception as exc:
            raise RuntimeError("No se pudo enlazar el tapichel al Placement de la escalera: %s" % exc)
    ...
    if master is not None:
        if bool(link_master_liner):
            ...
            # Dynamic minimal-demo mode: liner -> master.Placement is already a
            # dependency. Store only the stable Name on master to keep the graph DAG.
            set_prop(master, "App::PropertyString", "FA_OpeningLinerName", "FacilArquitectura", "Nombre tapichel lateral del buque", str(getattr(liner, "Name", "") or ""))
...
def create_native_stair(
    ...
    documentation_container=None,
    base_geometry_mode="part_feature",
    local_frame_source=None,
    placement_authority="source",
    context_links_mode="links",
):
    ...
    bases = _make_base_objects(doc, plan, geometry_mode=base_geometry_mode, local_frame_source=local_frame_source)
    width = float(plan["input"]["width_mm"])
    ...
    _tag_master(
        master, context, plan, railings_mode=railings_mode,
        context_links_mode=context_links_mode,
    )
    placement_mode = str(placement_authority or "source").strip().lower()
    if local_frame_source is not None:
        if placement_mode == "master":
            # The hidden source only defines the initial local frame. Afterwards the
            # visible native stair owns its Placement and can be moved directly.
            master.Placement = _global_placement(local_frame_source)
            set_prop(master, "App::PropertyString", "FA_PlacementSourceName", "FacilArquitectura", "Referencia inicial de Placement", str(getattr(local_frame_source, "Name", "") or ""))
            set_prop(master, "App::PropertyString", "FA_LocalFrameMode", "FacilArquitectura", "Marco local", "master_placement")
            set_prop(master, "App::PropertyBool", "FA_PlacementAuthority", "FacilArquitectura", "Placement autoritativo", True)
        else:
            set_prop(master, "App::PropertyLink", "FA_PlacementSource", "FacilArquitectura", "Fuente de Placement", local_frame_source)
            set_prop(master, "App::PropertyString", "FA_LocalFrameMode", "FacilArquitectura", "Marco local", "source_wire")
            try:
                master.setExpression("Placement", "%s.Placement" % local_frame_source.Name)
            except Exception as exc:
                raise RuntimeError("No se pudo usar el recorrido como Placement autoritativo de la escalera: %s" % exc)
    ...
    if local_frame_source is not None:
        for railing in railings:
            if placement_mode == "master":
                # Avoid an expression back to master: native stair segments already
                # reference these railings and such an expression can close a DAG.
                # Seed the same initial transform; runtime smoke will verify whether
                # FreeCAD's native stair hierarchy carries later master transforms.
                try:
                    railing.Placement = master.Placement
                except Exception:
                    pass
            else:
                set_prop(railing, "App::PropertyLink", "FA_PlacementSource", "FacilArquitectura", "Fuente de Placement", local_frame_source)
                try:
                    railing.setExpression("Placement", "%s.Placement" % local_frame_source.Name)
                except Exception:
                    # Railing visibility is secondary; the stair and slab opening remain
                    # authoritative even if a native FreeCAD railing rejects Placement expressions.
                    pass
    ...
    plan_placement_source = master if (local_frame_source is not None and placement_mode == "master") else local_frame_source
    plan_obj = _make_plan2d(doc, master, plan, local_frame_source=plan_placement_source) if bool(create_plan) else None
    if plan_obj is not None:
        if documentation_container is not None:
            # PLAN -> Owner -> Levels must not return through Level -> PLAN.
            # A document-level documentation group keeps the native owner link.
            add_to_container(documentation_container, plan_obj)
        elif lower_level is not None:
