...
Modified: 2026-09-09 22:09 America/Costa_Rica; canonical A1 demo commands added.
...
class ElectricCRWorkbench(Gui.Workbench):
    ...
    def Initialize(self):
        ...
        # Demo canonica A1: comando fino sobre el nucleo/adapter de prueba.
        # Se registra antes de construir barras y menus para que el mismo comando
        # quede disponible tanto por GUI como por el pequeno .FCMacro manual.
        demo_cmds = []
        try:
            from .commands import demo_electriccr as _demo_electriccr
            demo_cmds = _demo_electriccr.register_commands()
        except Exception as e:
            App.Console.PrintWarning(f"[ElectricCR][Demo] command_warning={e}\n")
        ...
        if demo_cmds:
            primary_toolbar_cmds.extend(demo_cmds)
            self.appendMenu("ElectricCR", demo_cmds)
        try:
