...
Descripcion: crea una cuadricula ArchGrid auxiliar ajustada a alineamientos reales del edificio.
Funcion principal: infiere ejes y una representacion recortada de referencia sin crear muros BIM.
Mantenimiento: la creacion de muros corresponde exclusivamente a FA_CreateWallsFromSketch.
...
from .bim_structure_utils import ensure_level_auxiliary_group, is_level
...
def _grid_output_container(doc, bim_group):
    """Return the auxiliary container for grid outputs when target is a Level."""
    if is_level(bim_group):
        return ensure_level_auxiliary_group(doc, bim_group)
    return bim_group
...
def create_inferred_building_grid(doc, bim_group, sources, params=None):
    ...
    _require_arch_grid()
    output_group = _grid_output_container(doc, bim_group)
    ...
    set_prop(
        ...
        "FA_GeneratesWalls",
        "FacilArquitectura",
        "La cuadricula crea muros BIM; debe permanecer False",
        False,
    )
    # La cuadricula es una herramienta auxiliar. No debe crear, ocultar ni sustituir
    # muros BIM. Los muros se generan de forma explicita con FA_CreateWallsFromSketch.
    try:
    ...
    return {
        "grid": grid,
        "display": display,
        # Claves conservadas por compatibilidad con consumidores historicos.
        "wall_base": None,
        "wall": None,
        "model": model,
    }
...
def remove_previous_inferred_grid(doc):
    """Remove only prior grid outputs and undo visibility side effects from legacy versions."""
    candidates = [
        obj
        for obj in list(getattr(doc, "Objects", []) or [])
        if str(getattr(obj, "FA_GeneratedBy", "") or "") == GENERATED_BY
    ]
    # Versions anteriores generaban un Wall reconstruido y ocultaban los muros
    # BIM originales. Si detectamos esa salida legacy, restauramos exclusivamente
    # los muros que aquella version podia haber ocultado antes de retirar la salida.
    if any(str(getattr(obj, "FA_Role", "") or "") == "reconstructed_wall" for obj in candidates):
        _restore_original_bim_walls(doc)
    role_priority = {
        "axis_system": 0,
        "axis_family": 1,
...
def _create_reconstructed_wall(
    ...
    """Legacy helper retained for compatibility; FA_CreateBuildingGrid no longer calls it."""
    edges = []
...
def _restore_original_bim_walls(doc):
    """Restore walls hidden by legacy FA_CreateBuildingGrid implementations."""
    restored = 0
    for obj in list(getattr(doc, "Objects", []) or []):
        ...
        if str(getattr(obj, "FA_Role", "") or "") != "wall":
            continue
        try:
            if not bool(obj.ViewObject.Visibility):
                obj.ViewObject.Visibility = True
                restored += 1
        except Exception:
            pass
    if restored:
        msg("Muros BIM originales restaurados tras retirar reconstruccion legacy: %d" % restored)
    return restored
